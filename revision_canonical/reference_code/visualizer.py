"""
Token Prediction Visualization Module.
Uses NetworkX and Matplotlib to visualize LLM token probabilities.
"""
...
# Ensure env vars are loaded if this module is imported
...
class TokenPredictor:
    def __init__(self, model_name: str, client: OpenAI = None):
        """
        Initialize predictor. Can pass an existing client or create a new one.
        """
    ...
    def predict_tokens(self, prompt: str, max_tokens: int = 100) -> List[Dict]:
        ...
        for chunk in response:
            if chunk.choices and chunk.choices[0].delta.content:
                ...
                # Some chunks might not have logprobs if it's just a delta update without token
                if not chunk.choices[0].logprobs:
                    continue
                ...
                top_token = token
                # Handle case where the generated token might not be in top 3 logprobs (rare edge case)
                top_prob = logprob_dict.get(token, -999.0)
                ...
                prediction = {
                    "token": top_token,
                    "probability": math.exp(top_prob) if top_prob != -999.0 else 0.0,
...
def visualize_predictions(G: nx.DiGraph, figsize=(14, 80)):
    ...
    # Handle cases with empty graphs
    if not G.nodes():
        return plt
    ...
    labels = {node: f"{G.nodes[node].get('token','')}\n{G.nodes[node].get('prob','')}" for node in G.nodes()}
